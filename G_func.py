...
class g_learn:
    def __init__(self, num_steps, num_risky_assets, window_size, x_vals_init, reward_params, trajs_num):
        self.num_risky_assets = num_risky_assets
        self.num_steps = num_steps
        ...
        # lambd = 0.001
        lambd = reward_params[0]
        # omega = 1.0
        omega = reward_params[1]
        beta = 1000.0
        gamma = 0.95
        # eta = 1.5  # 1.3 # 1.5 # 1.2
        eta = reward_params[2]
        ...
        self.x_t_multi = [self.x_vals_init[:]] * self.trajs_num

        self.x_next = 0
        self.x_next_multi = [0] * self.trajs_num
        self.u_t = 0
        self.u_t_multi = [0] * self.trajs_num

        self.trajs = [[]]*self.num_steps
        self.trajs_all = []
        self.trajs_multi = [[[]]*self.num_steps for i in range(self.trajs_num)]

        self.data_xvals = torch.zeros(self.num_steps, self.num_assets, dtype=torch.float64, requires_grad=False)
        self.data_uvals = torch.zeros(self.num_steps, self.num_assets, dtype=torch.float64, requires_grad=False)

        self.returns_all = []
        self.realized_rewards = torch.zeros(1, dtype=torch.float64, requires_grad=False)
        self.realized_G_fun = torch.zeros(1, dtype=torch.float64, requires_grad=False)
        self.realized_F_fun = torch.zeros(1, dtype=torch.float64, requires_grad=False)
    ...
    def run(self, t, exp_returns, sigma):
        ...
        for i in range(self.trajs_num):
            self.u_t_multi[i] = np.random.multivariate_normal(mu_t.detach().numpy(), self.learner.Sigma_prior[t,:].detach().numpy())
            self.x_next_multi[i] = self.x_t_multi[i] + self.u_t_multi[i]
        return weight

    def update(self, t, returns):
        returns = np.hstack((self.riskfree_rate, returns))
        ...
        for i in range(self.trajs_num):
            self.x_next_multi[i] = (1+returns)*self.x_next_multi[i]
            self.trajs_multi[i][t:] = [[]]*(self.num_steps - t)
            self.trajs_multi[i][t] = (self.x_t_multi[i], self.u_t_multi[i])
            self.x_t_multi[i] = self.x_next_multi[i]

        port_returns = (self.x_next.sum() - self.x_t.sum() - np.sum(self.u_t) - 0.015*np.abs(self.u_t).sum())/self.x_t.sum()
        # compute the rewards and realized values of G- and F-functions from the trajectory
        self.data_xvals[t,:] = torch.tensor(self.trajs[t][0], dtype=torch.float64)
        self.data_uvals[t,:] = torch.tensor(self.trajs[t][1], dtype=torch.float64)
        self.realized_rewards = torch.cat(
            (self.realized_rewards, torch.tensor([self.learner.compute_reward_on_traj(t, self.data_xvals[t,:], self.data_uvals[t,:])])),
            0
        )
        self.realized_G_fun = torch.cat(
            (self.realized_G_fun, torch.tensor([self.learner.compute_G_fun_on_traj(t, self.data_xvals[t,:], self.data_uvals[t,:])])),
            0
        )
        self.realized_F_fun = torch.cat(
            (self.realized_F_fun, torch.tensor([self.learner.compute_F_fun_on_traj(t, self.data_xvals[t,:])])),
            0
        )

        # rename
        self.x_t = self.x_next
        self.returns_all.append(port_returns)
        # end the loop over time steps

    def update_before_run(self, t):
        if t == 0:
            self.x_vals_init = self.x_t
            self.x_t_multi = [self.x_t for i in range(self.trajs_num)]
